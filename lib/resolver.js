...
module.exports = function (options) {
	...
	var resolveSchema = function (context, schema, prop, stack) {
		...
		_.each(schema, function(val, property){
			...
			if (property === '$ref') {
				...
				schema = resolveSchema(resolved.id ? normalizeReference(resolved.id, context) : context, resolved, prop, stack);
	...
	var normalizeReference = function (uri, context) {
		var components = uri.split('#'),
			...
			resolvedSchema = schemaID.length ? schemas[schemaID] : schemas[context];

		if (!resolvedSchema) {
			throw new ReferenceError('Bad URI: '+uri.red+' (in: '+context.yellow+')');
		}
	...
	var resolvePointer = function(uri, context) {
		...
		if (reference) {
			...
			throw new ReferenceError('Bad reference from '+context.bold+': '+uri.red);
		}
