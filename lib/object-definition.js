...
ObjectDefinition.prototype.build = function(object) {
  ...
  if (_.isArray(object.allOf)) {
    ...
    //   obj.example = this._formatter.format(exampleExtractor.generate(object, obj._original, {
...
ObjectDefinition.prototype.getExampleFromProperty = function(property) {
  ...
  var example = exampleExtractor.getExampleDataFromItem(propertySource);
  // If the source is a valid schema, and no example was provided, go resolve that
  if (propertySource.id && _.isUndefined(example)) {
    example = exampleExtractor.extract(propertySource);
  } else if (property.type === 'array') {
    example = [example];
  }
  // Stringify the example
  return this._formatter.format(example);
}
